    ...
    static partial class Win32Helpers
    {
        ...
        static unsafe uint GetBootDiskNumber()
        {
            ...
            if (!DeviceIoControl(
                ...
                out bytesReturned, // If lpOverlapped is NULL, lpBytesReturned cannot be NULL
                null
            ))
            {
                throw new Win32Exception();
        ...
        static unsafe string GetPhysicalDriveSerialNumber(uint driveNumber)
        {
            ...
            if (!DeviceIoControl(
                ...
                new ReadOnlySpan<byte>(&query, STORAGE_PROPERTY_QUERY.SizeOf(1)),
            ...
            try
            {
                if (!DeviceIoControl(
                        ...
                        IOCTL_STORAGE_QUERY_PROPERTY,
                        new ReadOnlySpan<byte>(&query, STORAGE_PROPERTY_QUERY.SizeOf(1)),
                        new Span<byte>((void*)descriptorPtr, (int)header.Size),
                        out bytesReturned,
                        null
                    ))
                {
